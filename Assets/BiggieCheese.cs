...
public class BiggieCheese : MonoBehaviour
{
    ...
    public void BossAI()
    {
        ...
        if(AiPhase == 2 || AiPhase == 3)
        {
            ...
            if(AiCounter > 80)
            {
                velo.y += rb.gravityScale * Time.fixedDeltaTime + 0.177f;
            }
            if(AiCounter % 6 == 0 && AiCounter > 100)
            {
                Projectile.NewProjectile<Cheese>(transform.position + new Vector3(0, 1.5f), new Vector2(0, 12f) + Utils.RandCircle(4) + rb.velocity * 1.0f, -1.4f);
            }
            velo.x *= 0.995f;
        }
        else
